          ...
          <div v-if="isEditingContrato" class="edit-mode">
            <div class="form-group">
              ...
              <select v-model="editForm.cajon" class="input-mini">
                ...
                <option v-if="cajonActual" :value="cajonActual.id">
                  {{ cajonActual.numero }} (Actual)
                </option>
                <option v-for="c in cajonesDisponibles" :key="c.id" :value="c.id">
                  {{ c.numero }} - {{ c.estado }}
                </option>
              </select>
            </div>
...
// Nueva variable para almacenar TODOS los cajones y poder llenar el select
const cajonesList = ref([]);

// Lógica de Edición
const isEditingContrato = ref(false);
const editForm = reactive({
  cajon: '',          // Cambiado: Ahora editamos el cajón
  monto_mensual: ''
});

// Computed para filtrar cajones en el select (Solo funcionales y no ocupados por otro contrato activo)
// Nota: Para simplificar, aquí mostramos los que tienen estado FUNCIONAL.
...
const cargarDatos = async () => {
  try {
    ...
    // Guardamos la lista completa de cajones en la variable reactiva
...
const iniciarEdicion = () => {
  if (contrato.value) {
    // Cargamos los valores actuales al formulario
...
const guardarEdicionContrato = async () => {
  try {
    // Usamos PATCH para actualizar Cajón y Monto
    ...
    await cargarDatos(); // Recargar para reflejar el nuevo cajón asignado
  } catch (error) {
    console.error(error);
    alert('Error al actualizar contrato. Verifica si el cajón está disponible.');
  }
};

// ... (Resto de funciones igual: subirArchivo, verImagen, formatDate) ...
const subirArchivo = async (event, tipo) => {
...
<style scoped>
/* ... (Mantén todos los estilos anteriores sin cambios) ... */
/* Solo asegúrate de tener estos estilos para el formulario que agregamos en el paso anterior */
...
/* ... (Resto de estilos generales, layout, lightbox, etc) ... */
...
.btn-icon-upload:hover { border-color: #7C5CFF; background: #F3F0FF; }
.btn-icon-upload.big { width: 60px; height: 60px; font-size: 1.5rem; margin: 0 auto 10px auto; }
